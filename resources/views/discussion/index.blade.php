  ...
  <div class="flex items-center gap-y-6 justify-between flex-wrap">
    ...
    <div class="flex xl:ml-auto sm:gap-x-1">
      ...
      @if (
        ...
        <a onclick="upVote(event, this);" title="up-vote discussion" class="ml-auto flex items-baseline text-lg lg:text-2xl font-semibold text-green-600 bg-green-100 pl-2 pr-4 py-0.5 rounded-full cursor-pointer">
      ...
      @else
        <a onclick="upVote(event, this);" href="{{ route('up-vote', ['discussion_id' => $discussion->id]) }}" title="up-vote discussion" class="ml-auto flex items-baseline text-lg lg:text-2xl font-semibold text-green-600 pl-2 pr-4 py-0.5 rounded-full cursor-pointer hover:bg-green-100">
          <svg class="self-center flex-shrink-0 h-8 w-8 text-green-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fill-rule="evenodd" d="M5.293 9.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L11 7.414V15a1 1 0 11-2 0V7.414L6.707 9.707a1 1 0 01-1.414 0z" clip-rule="evenodd" />
          </svg>
          <span class="sr-only"> Up-vote Discussion </span>
      ...
      @if (
        ...
        <a onclick="downVote(event, this);" title="down-vote discussion" class="flex items-baseline text-lg lg:text-2xl font-semibold text-red-600 bg-red-100 pl-2 pr-4 py-0.5 rounded-full cursor-pointer">
      ...
      @else
        <a onclick="downVote(event, this);" href="{{ route('down-vote', ['discussion_id' => $discussion->id]) }}" title="down-vote discussion" class="flex items-baseline text-lg lg:text-2xl font-semibold text-red-600 pl-2 pr-4 py-0.5 rounded-full cursor-pointer hover:bg-red-100">
          <svg class="self-center flex-shrink-0 h-8 w-8 text-red-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            <path fill-rule="evenodd" d="M14.707 10.293a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 111.414-1.414L9 12.586V5a1 1 0 012 0v7.586l2.293-2.293a1 1 0 011.414 0z" clip-rule="evenodd" />
          </svg>
          <span class="sr-only"> Down-vote discussion </span>
